...
class leaf {
  ...
  leaf(String colorName) {
    ...
    if (colorName=="red"){this.col = color(int(random(150,255))*colored,0,0, opacity);} // different variation of red
    if (colorName=="green"){this.col = color(0,int(random(50,150))*colored,0, opacity);} // different variation of red
    if (colorName=="blue"){this.col = color(0,0,int(random(50,150))*colored, opacity);} 
    age = 0;
    angle = random(-PI,PI);
  }
